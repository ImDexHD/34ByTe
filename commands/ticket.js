exports.run = function(client, message, args) {
 /* var server = message.guild;
  ...
  let OwnerRole = message.guild.roles.find("name", "Boss");
  let coownerRole = message.guild.roles.find("name", "Discord Manager");
  let managerRole = message.guild.roles.find("name", "Admin");
  let headAdminRole = message.guild.roles.find("name", "Super Moderator");
  let adminRole = message.guild.roles.find("name", "Moderator");
  let modRole = message.guild.roles.find("name", "Seller");
  let helRole = message.guild.roles.find("name", "Premium");
  ...
  if (arg == "new"){
    ...
    server.createChannel("ticket-" + name, "text").then(data => {
      ...
      data.setParent('527242769006198785');
  ...
  if(message.channel.name === 'ticket-' + message.author.id || message.member.roles.has(modRole.id) || message.member.roles.has(adminRole.id) || message.member.roles.has(headAdminRole.id) || message.member.roles.has(managerRole.id) || message.member.roles.has(coownerRole.id) || message.member.roles.has(OwnerRole.id)){
    ...
    if(message.channel.name === 'ticket-' + message.author.id || message.member.roles.has(modRole.id) || message.member.roles.has(adminRole.id) || message.member.roles.has(headAdminRole.id) || message.member.roles.has(managerRole.id) || message.member.roles.has(coownerRole.id) || message.member.roles.has(OwnerRole.id)){
      if(message.channel.parentID !== ("527242769006198785")) return message.reply("This is not a ticket.");
  ...
  if(message.member.roles.has(modRole.id) || message.member.roles.has(adminRole.id) || message.member.roles.has(headAdminRole.id) || message.member.roles.has(managerRole.id) || message.member.roles.has(coownerRole.id) || message.member.roles.has(OwnerRole.id)){
...
message.reply("This has currently been disabled please use https://www.34byte.it/forum/helpcenter.php instead!");
};
